...
- **Mobbin before new UI/UX.** For a new screen, flow, layout experiment, or any change that reshapes how the app feels, search Mobbin first and design from those real-app patterns. FitFight tokens still win for colour, type, and components. Skip Mobbin only when the change is small and the UI already exists (copy, spacing, a bug, a control we already use). If Mobbin is not signed in, stop and ask Marc to connect it. Do not invent the UI from memory.
