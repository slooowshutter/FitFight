...
- Version label shows at the **top of You** only (not the nav bar), e.g. `1.1.0 · build N · staging · 15 Sep`. Do not show it on Fights, New, Feed, or Feedback.
...
- Marketing version is **1.1.0**. Do not bump it unless Marc asks. Apple closed the 1.0.0 train (altool 90062 / 90186). Do not upload 1.0.0. CI still increments the **build number**. Changelog rows reuse `1.1.0` until Marc asks for the next version.
...
- First TestFlight upload **succeeded** (build `0.1.0 (1)`). Current release/TestFlight marketing version: **1.1.0**.
...
3. If users will see it: append a `ReleaseNote` in `Changelog.swift` using the current `MARKETING_VERSION` (`1.1.0`). Do **not** change `MARKETING_VERSION` in `project.pbxproj` unless Marc asks or Apple closed the train. CI bumps the build number.
