...
- Talk to Marc only for things only he can do: Apple login, GitHub secrets, TestFlight testers, legal. Agents cannot `workflow_dispatch`. App changes that land on `main` upload to TestFlight by themselves. He just opens TestFlight → Update.
...
- TestFlight on every app push to `main`, plus daily **18:00 UTC**, plus manual `workflow_dispatch` for a PR branch.
...
4. Don’t ask Marc to open Xcode or his Mac. After app changes land on `main`, TestFlight uploads itself. He opens TestFlight → Update.
