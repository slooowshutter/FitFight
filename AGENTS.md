...
- Version label shows at the **top of You** only (not the nav bar), e.g. `1.1.2 · build N · staging · 16 Sep`. Do not show it on Fights, New, Feed, or Feedback.
...
- Marketing version is **1.1.2**. Apple closed the 1.1.1 train (altool 90062 / 90186) after App Store approval. Do not upload 1.1.1 or 1.0.0. `FITFIGHT_RELEASE_VERSION` and Fastlane must equal 1.1.2. CI still increments the **build number**. Changelog rows reuse `1.1.2` until Marc asks for the next version.
...
- First TestFlight upload **succeeded** (build `0.1.0 (1)`). Next prepared TestFlight marketing version: **1.1.2**. Apple closed 1.1.1. See `docs/status.md` for availability.
...
3. If users will see it: append a `ReleaseNote` in `Changelog.swift` using the current `MARKETING_VERSION` (`1.1.2`). Do **not** change `MARKETING_VERSION` in `project.pbxproj` unless Marc asks or Apple closed the train. CI bumps the build number.
