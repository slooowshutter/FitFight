...
- **Collect Apple Health workouts before mapping sports.** Inventory is [`research/data-sources.md`](research/data-sources.md). Most watches and apps (Apple Watch, WHOOP, Garmin, Oura, Fitbit/Google Health, Strava, Nike Run Club, …) already land in HealthKit; they are not 40 OAuth projects. First slice: private `HKWorkout` plus duration, distance, and active energy, with sport type from Apple’s `HKWorkoutActivityType`. Do not score Fights from this yet. Consent, HealthKit usage strings, App Privacy labels, retention, and deletion must cover workouts before the phone sends them. Direct WHOOP/Oura/Strava APIs stay later; Strava’s API is blocked for shared leaderboards without written approval.
