...
- Each fight has Stats, Feed, Details, and History for recurring fights with earlier rounds. Details shows the current round's start and end dates/times in the Fight time zone, its creator when available, and the current participant count. Pending invites and people waiting for the next round are excluded from that count. Share, copy link, and copy code live inside Details when an invite code exists.
...
- Apple Health sends merged Steps totals for active Fights. With permission, FitFight also imports accessible merged daily movement history and workout summaries into a private activity pipeline. Supported individual HealthKit samples and explicit deletions are retained privately alongside merged totals. Other activity types are not Fight options yet.
- Settings keeps Refer a friend, Preferences, Privacy, Support, Versions, Sign out, and Delete account. Preferences sits below Refer a friend and saves language (Follow iPhone, English, French) and appearance (Follow iPhone, Light, Dark) to the account. Light and Dark use the existing Day and Night palettes. Notifications and Beta testing are inside Preferences, alongside installation source, version/build, and account environment. Beta testing keeps the TestFlight and App Store links and explains replacement and separate accounts, fights, progress, and preferences. Bugs & requests remains on Feedback, with a shortcut on You.
- Companion: Change animal opens the animal grid. Make it yours lets people save a description or create one complete animal for EUR 4.99 through Apple. One purchase runs the portrait workflow followed by five fitness images, saves the original Blend URLs to the account, and survives an interrupted session. A selected character changes on Fights, New and You with today's Steps at 2,000, 4,000, 6,000 and 8,000. The credit-based AI routes and group-photo adapter remain in the backend for compatibility, but the new character screen uses the paid pair. Purchases and generation remain disabled on staging until Apple and Blend configuration, hosted checks, and Sandbox verification are complete.
