...
The living list is [`backlog.md`](backlog.md). Right now the screens are a design-accurate mock with fixture people. No HealthKit, no backend, no notifications. Don’t invent the gaps listed in [`design/source/INVENTORY.md`](design/source/INVENTORY.md). When real fights start, follow [`sync.md`](sync.md) — do not put `URLSession` on a view.
