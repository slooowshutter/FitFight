...
- **No:** do **not** take “average run speed” and add 10–20% for an intermediate, or subtract 10% for a long run. Recreational average pace is usually already too fast (the gray zone). A long run is almost the **same speed as easy**, just **longer**. Tempo is about **+20% speed vs easy**, not vs average. Intervals are about **+30% speed vs easy**.
- **Same sport family, different adapters:** running, cycling, and swimming all use easy / threshold / VO2 sessions and an 80/20-ish mix at 5+ days. The **anchor** changes (run pace, bike power, swim CSS). Running is the only one where a too-long session is a major injury spike.
...
The same +20% vs easy holds at VDOT 40 and 60. So “tempo ≈ easy speed × 1.20” is a decent **fallback** when you only know easy pace.

It fails if the baseline is **average GPS pace**, because that average is a stew of easy, commute, and accidental tempo. Then “average + 20%” becomes interval work every time you wanted a moderate day.
...
1. Anchor on a recent all-out 5K/10K (or estimated race time). Derive easy / threshold / interval from that one number. Runna does exactly this.
2. If there is no race, take the **slowest** 20–40 min steady runs from the last 4 weeks as easy. Do **not** average everything.
...
Runna is a **rules-based coaching system**, not a published neural net. Public support docs are enough to copy the product loop. Internal mileage curves stay black-box.
...
Beginner / return-to-running plans are **effort-based**. No pace targets. Walk-run first.
...
- **Long run has its own build limit**, independent of weekly mileage. After 2+ weeks without a long run, the first one back is capped.
- Foundation plans use a **double constraint**: min(fixed bump, multiple of current best).
- Deload weeks cut **hard sessions more than easy ones**.
- “Not feeling 100%” **pauses mileage and pace progression**.
- They will not let you race a distance your current long run cannot build toward.
- Pace is preferred over HR because wrist HR is noisy and needs a lab to set zones well.
...
| 2–3 | ~60/40 is more realistic: 1 easy + 1–2 quality. Strict 80/20 would mean almost never going hard. |
...
Seiler’s review: nationally competitive endurance athletes cluster around **~80% of sessions easy, ~20% dominated by high intensity**. Polarized means most of that 20% is Z3 with little Z2. Pyramidal means the 20% is more Z2 than Z3. Both beat “everything at threshold.” ([Seiler 2010](https://www.edzo.info.hu/images/Seiler2010.pdf); [Stöggl & Sperlich 2015](https://www.frontiersin.org/journals/physiology/articles/10.3389/fphys.2015.00295/full); [Casado et al. 2022](https://journals.humankinetics.com/view/journals/ijspp/17/6/article-p820.xml); [Haugen et al. 2022](https://pmc.ncbi.nlm.nih.gov/articles/PMC8975965/))
...
From `fitness`, set three **speed** multipliers vs easy (the fallback when we do not ship a full VDOT table):

| Band | Speed vs easy | Pace vs easy | Use |
|---|---|---|---|
| Easy / long | 0.95–1.05 | same or 0–20 s/km slower | Z1 |
| Tempo | **1.18–1.22** | ~20% faster | Z2 |
| Interval | **1.28–1.33** | ~30% faster | Z3 |

That is Marc’s “percent of a baseline,” with the baseline fixed as **easy**, not average.

If we later add Daniels tables, replace the multipliers with VDOT E / T / I. The week builder does not care.
...
|---|---|---|---|
| 2 | 1 quality, 1 easy-or-long | 50/50 | Runna 60/40. Alternate tempo and intervals across weeks. Long only if the quality day is not yesterday. |
| 3 | 1 long, 1 quality, 1 easy | 33/67 | Default amateur week |
| 4 | 1 long, 1 quality, 2 easy | 25/75 | First week that looks like 80/20 |
| 5 | 1 long, 1–2 quality, rest easy | 20–40 / 60–80 | Second quality only if they chose “challenging” |
| 6 | 1 long, 2 quality, 3 easy | ~33/67 | Build **weekly km slower** than a 3-day runner |

Default difficulty = **balanced** = 1 quality/week at 2–4 days, 2 quality/week only at 5–6 days.
...
- Tue quality: 10 easy + 25 tempo at **5:00/km** (+20% speed) + 10 easy
...
- Z1 / Z2 / Z3
- Most sessions easy at 5+ days; more quality % at 2–3 days
...
|---|---|---|---|
| Anchor | Race pace / VDOT | FTP (watts), often 95% of 20-min test | CSS from 400 m and 200 m times |
| Easy / tempo / VO2 metric | min/km | %FTP (Coggan Z2 ~56–75%, Z4 ~91–105%) | s/100 m vs CSS |
| Long session | Time on feet. Muscles and tendons limit it | Hours. 2–4 h is normal | Technique and shoulders. Volume in metres |
| Injury | Session distance spike is the proven risk | Overuse exists; impact is low. Volume can be 2–3× running | HR in water is a bad driver. Stroke errors look like “fitness drop” |
| Substitute for an easy run? | — | Yes, for aerobic Z1 | Yes, for aerobic Z1 |
| Substitute for a long run? | — | **No** for run-specific durability | **No** |
...
|---|---|---|---|---|
| **Runna** | Full adaptive week, race or habit | Instant Workout = user picks type | Estimated current race time + opt-in Pace Insights | Loop, constraints, 80/20-by-days |
| **Garmin Daily Suggested** | Rolling 7 days | **Auto-picks today**, including rest | VO2 / load / recovery on-watch | Today picker |
| **Garmin Coach** | Goal race, chosen days | Syncs structured workouts | Performance + recovery | Too watch-heavy for us |
| **Nike Run Club** | Fixed 5K–marathon PDFs | Audio library; not from your history | Coach-written, not your VDOT | Tone, not math |
| **TrainingPeaks** | Coach-built, TSS / CTL | Follow the calendar | FTP / threshold | Too pro |
| **Strava** | Segments / social | Not a coach | Relative Effort is load, not a session type | Skip |
...
- Casado A et al. *Training Periodization, Methods, Intensity Distribution, and Volume in Highly Trained and Elite Distance Runners.* IJSPP 2022. https://journals.humankinetics.com/view/journals/ijspp/17/6/article-p820.xml
