...
Sibling docs (other PRs may land them): [`sync.md`](sync.md) owns the GRDB
replica, outbox, and HealthKit **compile** path; [`architecture.md`](architecture.md)
owns type layers. **This file wins** on identity, authz, secrets, abuse, retention,
and **what may leave the phone**. Sync wins on queue mechanics (idempotency, SWR,
outbox states). If architecture says `CanonicalSample` is uploaded, ignore that
row — only daily totals go on the wire. If sync shows `FIGHT-742` as a column
example, that is the fixture; the generator is below.
...
| TestFlight | **`main` + daily cron only.** PRs keep Simulator + screenshots. Unreviewed Fastlane on a branch must not see the `.p8`. |
...
- `day` is a calendar date in the timezone **frozen at join**.
...
- A timezone change after join (frozen at join, or people shop DST)
