...
Vercel also needs `CRON_SECRET` (Preview + Production). Vercel Cron sends it as `Authorization: Bearer …` to `/api/internal/close-fights` once daily at **03:00 UTC**, which is compatible with Hobby. **Hosted Supabase Cron** on develop and production should call the same route every **15 minutes** for ending reminders, the single conditional final-sync request, and evening social summaries. Verify that job is active in each environment before the [notification rollout](notifications.md); the daily Vercel job alone cannot deliver evening summaries on time. Opening the app also closes due fights and drains the notification outbox. Never put this value in git or chat.
