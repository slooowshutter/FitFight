...
Read this before building. Last updated **27 Aug 2026**. App: **0.9.0**. Git: **#32** merged `develop` → `main` (production). Staging host is persistent develop `zstzbf…`.
...
Staging (this PR / `develop`): TestFlight → **Update**. Line says **staging**. Sign in, username, Apple Health, add a friend, New → 3d → Start **once**. If sign-in fails: new develop Auth → Apple → On, client ID `com.fitfight.mvp`. Friends: Internal Testing, their own Apple IDs.

Production (`main`, after #32): Update a **prod** build. Same loop. Apple On on **production** Auth if sign-in fails. Extra Start taps are leftover test rows.
...
| Create Steps fight | Phone writes the fight to Supabase. Starts **now**, not tomorrow. Start shows Starting… |
