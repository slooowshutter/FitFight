...
| Accept / Join | Invites still accept in the fight. Joinable fights open the same Accept/Join screen from a code, the live list, or a shared link. Joins go through the server. After accept or join, the phone uploads Apple Health Steps from the original fight start so a late join is not stuck at 0. Leave a joinable or repeating fight from the fight itself so the next window does not copy you in. |
