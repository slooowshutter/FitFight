...
Read this before building. Last updated **28 Aug 2026**. App: **0.9.0**. Staging host `zstzbf…`.
...
The old staging host is gone. A leftover Apple session, or a missing `profiles` row, left the phone with no way out.

This TestFlight **Sign out**s that screen (and drops a session from the dead host). Update, then Sign in with Apple. Do not merge to `main`.
