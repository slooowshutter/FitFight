...
First isolated cloud checks on `3ceb50c` passed web typecheck and the full backend
suite, disposable migrations/lint/RLS/transaction tests, legacy build 113 checks,
and the GitHub-hosted macOS simulator build. The native check also compiles frozen
identical feedback decoders from staging build 201 (`d97145a`) and production build
202 (`e2783be`) against old and new response fixtures. The combined Profile dependency
and additional authenticated HTTP regressions are being rechecked before completion.
