...
Map of the rest of the design: [`system/README.md`](system/README.md). Types/SoD: [`system/layers.md`](system/layers.md). HealthKit compile: [`system/ingest.md`](system/ingest.md). Postgres: [`system/schema.md`](system/schema.md).
...
Grace: until **next noon in the fight IANA timezone** after `window_end`, hard cap **48 hours** (server constant). Visible push at `window_end`:
...
sequenceDiagram
    ...
    Job->>API: status = grace, grace_until = next noon in fight TZ (cap 48h)
