...
schema accepts the disposable rehearsal project, `qkkhkfepjhdgowmhhpyf`, and
production, `pvqntpteehdvhqyctwum`. Marc authorized the production rollout and
submission on 16 September 2026. The temporary token and expiry still restrict
each deployment to its explicitly configured target.
...
`zstzbfocunthczzubggz` and the authorized target. Supabase-managed database and
...
Apply `archive-bucket.sql` only on the authorized target. The bucket is private
...
4. `{"action":"apply","run_id":"..."}` commits the same transaction on the
   authorized target. Repeating the same run must report `already_applied: true`.
...
when the transfer finishes. Remove the ignored local token file. Retain the
private cloud checkpoint for the final catch-up and release verification.
...
final beta cutoff. The initial production import occurs before review. Beta
remains usable during review, so repeat the catch-up immediately before the
manual public release. The rehearsal alone does not authorize live changes.
