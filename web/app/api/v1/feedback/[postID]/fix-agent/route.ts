import { ApiError, ERROR_CODES, apiRoute, corsPreflight, json, readJson, requireUuid } from "@/lib/http";
...
export const POST = apiRoute<{ postID: string }>(async (request, { params }) => {
  ...
  const parsed = launchFeedbackFixRequestSchema.safeParse(await readJson(request));
  if (!parsed.success) {
    throw parsed.error;
  }
  ...
  const launched = await launchFeedbackFixAgent(detail, fetch, senderMetadata);
