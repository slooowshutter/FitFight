...
import { listFightPostReactionPeople, setFightPostReaction } from "@/lib/supabase/queries/fight-post-engagement-supabase-query";
import { processNotificationOutbox } from "@/lib/supabase/queries/process-notification-outbox-supabase-query";
import { listFightPostReactionPeopleQuerySchema, setFightPostReactionRequestSchema } from "@/lib/types/feed/fight-post";
...
export const GET = apiRoute<{ postID: string }>(async (request, { params }) => {
  ...
  return json(await listFightPostReactionPeople(userId, requireUuid(params.postID, "postID"), parsed.data));
});
