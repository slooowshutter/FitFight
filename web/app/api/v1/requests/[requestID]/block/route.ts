...
import { blockFeatureRequestAuthor } from "@/lib/supabase/queries/feature-requests-supabase-query";
...
export const POST = apiRoute<{ requestID: string }>(async (request, { params }) => {
  ...
  const result = await blockFeatureRequestAuthor(userId, requestId);
  return json(result);
});
