...
function appleSignInEnvironment(): AppleSignInEnvironment {
    ...
    if (!parsed.success) {
        throw new ApiError(
            500,
            ERROR_CODES.config,
            "Sign in with Apple server credentials are missing",
        );
    }
    return parsed.data;
}
...
export function decryptAppleRefreshToken(
    token: EncryptedAppleRefreshToken,
): string {
    const environment = appleSignInEnvironment();
    try {
        ...
        throw new ApiError(
            ...
            "Stored Apple authorization could not be read",
        );
    }
}
...
export async function exchangeAppleAuthorizationCode(
    ...
    if (!response.ok) {
        throw new ApiError(
            ...
            "Apple authorization could not be saved",
        );
    }
