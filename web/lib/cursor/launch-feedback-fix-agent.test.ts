...
test("starts a v1 cloud agent on develop without a webhook", async () => {
    ...
    try {
        ...
        assert.match(
            prompt,
            /There was a request\/bug from the Bugs & requests tab/,
        );
        ...
        assert.doesNotMatch(
            prompt,
            /https:\/\/github.com\/slooowshutter\/FitFight/,
        );
        assert.equal(
            fitFightGithubRepoUrl,
            "https://github.com/slooowshutter/FitFight",
...
test("labels a feature request and still attaches device metadata with no comments", async () => {
    ...
    try {
        ...
        await launchFeedbackFixAgent(featureDetail, (async (_url, init) => {
            prompt = (
                JSON.parse(String(init?.body)) as { prompt: { text: string } }
            ).prompt.text;
            return v1CreatedResponse(agentUrl);
        }) as typeof fetch);
...
test("includes photo, video, and file links in the agent prompt", async () => {
    ...
    try {
        ...
        assert.match(
            prompt,
            /blank-chart\.jpg \(photo, image\/jpeg\) https:\/\/example.com\/blank-chart\.jpg/,
        );
        assert.match(
            prompt,
            /console\.log \(file, text\/plain\) https:\/\/example.com\/console\.log/,
        );
    ...
    try {
        await assert.rejects(
            () =>
                launchFeedbackFixAgent(detail, (async () => {
                    return new Response(
                        JSON.stringify({ message: "cloud agents unavailable" }),
                        { status: 502 },
                    );
                }) as typeof fetch),
            (error: unknown) =>
    ...
    try {
        await assert.rejects(
            ...
            (error: unknown) =>
                ...
                error.code === "internal" &&
                error.message ===
                    "Cursor rejected the API key on Vercel Preview.",
        );
    } finally {
        restoreEnv("CURSOR_API_KEY", previous);
    }
    ...
    try {
        await assert.rejects(
            ...
            (error: unknown) =>
                ...
                error.status === 502 &&
                error.message ===
                    "Cursor can’t access the FitFight GitHub repo with this API key.",
        );
    } finally {
        restoreEnv("CURSOR_API_KEY", previous);
    }
