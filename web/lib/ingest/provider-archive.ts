...
export async function readProviderArchive(
    ...
    const consume = async (line: string) => {
        ...
        if (new TextEncoder().encode(text).byteLength > 1_000_000) {
            throw new ApiError(
                400,
                ERROR_CODES.archive_invalid,
                `Archive line ${lineNumber} is too large`,
            );
        }
        let value: unknown;
        try {
            ...
            throw new ApiError(
                400,
                ERROR_CODES.archive_invalid,
                `Archive line ${lineNumber} is invalid JSON`,
            );
        }
        ...
        if (checkpointSeen) {
            throw new ApiError(
                400,
                ERROR_CODES.archive_invalid,
                "Checkpoint must be the final record",
            );
        }
    ...
    if (bytes !== expectedBytes) {
        throw new ApiError(
            ...
            "Archive byte size does not match",
        );
    }
