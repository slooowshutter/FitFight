...
import {
    createAppFeedbackBacklogItem,
    markAppFeedbackBacklogStatus,
} from "./create-app-feedback-item";
import {
...
test("creates a P0 FitFight App feedback Inbox row for a bug", async () => {
    ...
    try {
        ...
        assert.match(
            ...
            /@maya_moves · bug · staging/,
        );
        assert.match(
            properties.Notes.rich_text[0]?.text.content ?? "",
            /feedback_post: dddddddd-dddd-4ddd-8ddd-dddddddddddd/,
        );
        assert.equal(
            properties.Priority.select.name,
            notionAppFeedbackDefaults.priority,
        );
        assert.equal(
            properties.Product.select.name,
            notionAppFeedbackDefaults.product,
        );
        assert.equal(
            properties.Source.select.name,
            notionAppFeedbackDefaults.source,
        );
        assert.equal(
            properties.Status.select.name,
            notionAppFeedbackDefaults.status,
        );
...
test("maps a feature request to Type Feature and does not fail the post when Notion errors", async () => {
    ...
    try {
        const created = await createAppFeedbackBacklogItem(feature, (async (
            _url,
            init,
        ) => {
    ...
    try {
        const updated = await markAppFeedbackBacklogStatus(
            ...
            (async (url, init) => {
                            ...
                            results: [
                                {
                                    ...
                                    properties: {
                                        Notes: {
                                            rich_text: [
                                                {
                                                    plain_text: `feedback_post: ${post.id}`,
                                                },
                                            ],
                                        },
                                    },
                ...
                assert.match(
                    body.properties.Notes.rich_text[0]?.text.content ?? "",
                    new RegExp(prUrl),
                );
                return new Response(
                    JSON.stringify({
