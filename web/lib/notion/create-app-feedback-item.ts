                ...
                properties: {
                    ...
                    Notes: {
                        rich_text: [
                            {
                                text: {
                                    content: notes.slice(0, RICH_TEXT_LIMIT),
                                },
                            },
                        ],
                    },
                    Priority: {
                        select: { name: notionAppFeedbackDefaults.priority },
                    },
                    Product: {
                        select: { name: notionAppFeedbackDefaults.product },
                    },
                    Source: {
                        select: { name: notionAppFeedbackDefaults.source },
                    },
                    Status: {
                        select: { name: notionAppFeedbackDefaults.status },
                    },
...
export async function markAppFeedbackBacklogStatus(
    ...
    try {
        ...
        const parsed = notionFeedbackPageQuerySchema.safeParse(
            await query.json(),
        );
