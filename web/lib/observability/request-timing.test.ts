...
test("timed requests return measured phases and log no request payloads or credentials", async (t) => {
    ...
    const response = await route(
        new Request(
            "https://fitfight.app/api/v1/healthkit/steps?secret=private-query",
            {
                method: "POST",
                body: "private-health-payload",
                headers: {
                    Authorization: "Bearer test-credential",
                    "X-FitFight-Trace-ID": traceId,
                },
            },
        ),
...
test("failed phases keep timing headers and invalid trace identifiers cannot enter logs", async (t) => {
    ...
    assert.match(
        response.headers.get("X-FitFight-Trace-ID") ?? "",
        /^[0-9a-f-]{36}$/,
    );
    assert.match(
        response.headers.get("Server-Timing") ?? "",
        /db;dur=[\d.]+, total;dur=[\d.]+/,
    );
    assert.ok(
        !JSON.stringify(log.mock.calls[0].arguments).includes(
            "untrusted-user-text",
        ),
    );
});
