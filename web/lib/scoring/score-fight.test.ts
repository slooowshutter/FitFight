...
test("highest total ranks members and splits ties deterministically", () => {
    ...
    assert.deepEqual(
        winner.map(({ userId, rank, outcomeMinor }) => ({
            userId,
            rank,
            outcomeMinor,
        })),
    ...
    assert.equal(
        tie.reduce((sum, row) => sum + row.outcomeMinor, 0),
        999,
    );
...
test("proportional and goal scoring allocate the complete pot", () => {
    ...
    assert.equal(
        proportional.find((row) => row.userId === "a")?.outcomeMinor,
        133,
    );
    assert.equal(
        proportional.find((row) => row.userId === "b")?.outcomeMinor,
        67,
    );
...
test("final forfeit: three people use two-tier rank regardless of input order", () => {
    ...
    assert.deepEqual(
        rows.map(({ userId, currentValue, rank }) => ({
            userId,
            currentValue,
            rank,
        })),
...
test("final scoring matches live scoring when every accepted member is complete", () => {
    ...
    assert.deepEqual(
        scoreFightAtFinal({
            ...args,
            members: members.map((member) => ({ ...member, complete: true })),
        }),
