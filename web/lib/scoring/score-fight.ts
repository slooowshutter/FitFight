...
export function fightPot(
    stakeMinor: number | null | undefined,
...
function goalTarget(
    ...
    if (
        member.personalTarget !== null &&
        Number.isFinite(member.personalTarget)
    ) {
    ...
    switch (input.outcomeRule) {
        case "highest_total": {
            ...
            const winners = members
                .filter((member) => member.value === top)
                .map((m) => m.userId);
            const split =
                top > 0 ? splitEvenly(pot, winners) : new Map<string, number>();
            for (const member of members) {
...
export function scoreFightAtFinal(input: {
    ...
    switch (input.outcomeRule) {
        ...
        case "hit_your_goal": {
            ...
            const incomplete = input.members.filter(
                (member) => !member.complete,
            );
            if (complete.length === 0) {
                return scoreFight(input).map((row) => ({
                    ...row,
                    rank: 1,
                    outcomeMinor: 0,
                }));
            ...
            return [
                ...
                (a, b) => a.rank - b.rank || a.userId.localeCompare(b.userId),
            );
        }
        default: {
            const exhaustive: never = input.outcomeRule;
