...
export async function acceptInvite(
    ...
    const { data: inviteData, error: inviteError } = await admin
        .from("fight_invites")
        .select(
            "id, fight_id, invited_user_id, token_hash, expires_at, revoked_at, accepted_at",
        )
    ...
    if (inviteUpdateError) {
        throw new ApiError(
            500,
            ERROR_CODES.db_error,
            "Could not mark invite accepted",
        );
    }

    if (
        memberState === "accepted" &&
        ["live", "scheduled", "awaiting_final_sync"].includes(fight.state)
    ) {
