...
function withSupabaseEnv(t: { after: (fn: () => void) => void }) {
  ...
  process.env.NEXT_PUBLIC_SUPABASE_URL = process.env.NEXT_PUBLIC_SUPABASE_URL || "https://cancel-fight.example";
...
test("a failed series pause does not cancel the fight", async (t) => {
  withSupabaseEnv(t);
  let fightPatches = 0;
  t.mock.method(globalThis, "fetch", async (input: RequestInfo | URL, init?: RequestInit) => {
    const request = new Request(input, init);
    ...
    switch (url.pathname) {
      ...
      case "/rest/v1/fight_series":
        assert.equal(request.method, "PATCH");
        return Response.json({ message: "pause failed" }, { status: 500 });
      default:
        throw new Error(`Unexpected query: ${url.pathname}`);
    }
  });
