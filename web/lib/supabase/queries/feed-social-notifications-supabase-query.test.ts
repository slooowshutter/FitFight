...
test("notification preference patches need at least one toggle", () => {
  ...
  assert.equal(updateNotificationPreferencesRequestSchema.safeParse({}).success, false);
...
test("notification preference routes authenticate before reading or writing", async () => {
  ...
  const get = await GET(new Request("https://staging.fitfight.app/api/v1/notifications/preferences"), context);
  const patch = await PATCH(new Request("https://staging.fitfight.app/api/v1/notifications/preferences", {
    method: "PATCH",
  }), context);
...
test("a reply to the post author’s comment is one reply, not also a comment", async () => {
  ...
  assert.equal(inserted.length, 1);
  assert.equal((inserted[0] as { kind: string; user_id: string }).kind, "comment_reply");
  assert.equal((inserted[0] as { user_id: string }).user_id, otherId);
});
...
test("a reply notifies the parent commenter instead of sibling commenters", async () => {
  ...
  const kinds = (inserted as Array<{ user_id: string; kind: string }>).map((row) => `${row.user_id}:${row.kind}`).sort();
  assert.deepEqual(kinds, [`${otherId}:post_comment`, `${thirdId}:comment_reply`]);
  assert.equal(queries.some((sql) => sql.includes("fight_post_comments") && sql.includes("parent")), false);
});
