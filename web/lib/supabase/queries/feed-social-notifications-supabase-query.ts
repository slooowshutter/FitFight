...
async function recipientRows(sql: Sql, actorId: string, userIds: string[]): Promise<RecipientRow[]> {
...
async function insertSocialIntents(
  ...
  const rowsByUser = new Map(
    (await recipientRows(sql, actorId, [...new Set(wanted.map((row) => row.userId))])).map((row) => [
      row.user_id,
      row,
    ]),
  );
