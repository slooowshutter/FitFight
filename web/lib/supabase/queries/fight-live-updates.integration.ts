  ...
  await new Promise<void>((resolve, reject) => {
    ...
    channel.subscribe((status, error) => {
      if (status === "SUBSCRIBED" || status === "CHANNEL_ERROR" || status === "TIMED_OUT") {
        ...
        if (allowed === (status === "SUBSCRIBED") && status !== "TIMED_OUT") resolve();
        else reject(error ?? new Error(`Unexpected subscription status: ${status}`));
      }
    });
  });
}

test("private fight invalidations follow commits and reconcile both participants", { timeout: 60_000 }, async (t) => {
  const clients = Array.from({ length: 3 }, () => createClient(env.SUPABASE_TEST_URL, env.SUPABASE_TEST_ANON_KEY, {
    auth: { persistSession: false, autoRefreshToken: false },
  }));
  const users: string[] = [];
  ...
  const channels = clients.map((client, index) => client
    .channel(`fitfight:fights:${users[index]}`, {
      config: { private: true, broadcast: { ack: true, replication_ready: true } },
    })
    .on("system", {}, (message: unknown) => {
      assert.ok(message && typeof message === "object" && "status" in message);
    ...
    .on("broadcast", { event: "fights_changed" }, (message) => {
      ...
      assert.ok(![fightId, ...users].includes(message.payload.id));
      received[index]++;
    }));
  await Promise.all(channels.map((channel) => subscribe(channel, true)));
  for (let attempt = 0; attempt < 100 && replicationReady.size < 3; attempt++) await delay(50);
  assert.equal(replicationReady.size, 3, "Each channel must report that database replication is ready");
  const denied = clients[2].channel(`fitfight:fights:${owner}`, { config: { private: true } });
  ...
  await subscribe(anonymous.channel(`fitfight:fights:${owner}`, { config: { private: true } }), false);
  ...
  await delay(500);
  received.fill(0);
  const before = await database`select count(*)::int as count from realtime.messages
  ...
  await assert.rejects(database.begin(async (sql) => {
    ...
    const [pending] = await sql`select count(*)::int as count from realtime.messages
      where topic = ${"fitfight:fights:" + peer} and event = 'fights_changed'`;
  ...
  const [rolledBack] = await database`select current_value::int as value from public.fight_members
    where fight_id = ${fightId} and user_id = ${owner}`;
  assert.equal(rolledBack.value, 8000);
  const [afterRollback] = await database`select count(*)::int as count from realtime.messages
    where topic = ${"fitfight:fights:" + peer} and event = 'fights_changed'`;
  assert.equal(afterRollback.count, before[0].count);

  const cutoff = new Date().toISOString();
  await syncHealthKitAggregates(owner, {
    complete_through: cutoff, time_zone: "UTC", merged_days: [],
    fight_aggregates: [{ fight_id: fightId, starts_at: startsAt, ends_at: endsAt, cutoff_at: cutoff, steps: 20000 }],
  }, database);
  for (let attempt = 0; attempt < 100 && (received[0] === 0 || received[1] === 0); attempt++) await delay(50);
  assert.ok(received[0] > 0 && received[1] > 0, "Both signed-in phones must receive the committed invalidation");
  assert.equal(received[2], 0, "An unrelated user must receive no fight activity");
  const [confirmed] = await database`select count(*)::int as count from realtime.messages
    where topic = ${"fitfight:fights:" + peer} and event = 'fights_changed'`;
  assert.ok(confirmed.count > before[0].count, "Successful writes must actually enqueue a message");
  const snapshots = await Promise.all([owner, peer].map((user) => readFightSnapshot(user, "UTC", database)));
  assert.deepEqual(snapshots[0].members, snapshots[1].members);
  assert.equal(snapshots[1].members.find((member) => member.user_id === owner)?.current_value, 20000);
  ...
  const { POST: refresh } = await import("@/app/api/v1/fights/refresh/route");
  const { POST: snapshot } = await import("@/app/api/v1/fights/snapshot/route");
  const { data: session } = await clients[0].auth.getSession();
  assert.ok(session.session);
  for (const client of [{ version: "1.0.0", build: "190" }, { version: "1.1.0", build: "200" }]) {
    const response = await refresh(new Request("http://localhost/api/v1/fights/refresh", {
      method: "POST", body: JSON.stringify({ time_zone: "UTC" }),
      headers: {
        Authorization: `Bearer ${session.session.access_token}`,
        "X-FitFight-Version": client.version, "X-FitFight-Build": client.build,
      },
  ...
  const liveResponse = await snapshot(new Request("http://localhost/api/v1/fights/snapshot", {
    method: "POST", body: JSON.stringify({ time_zone: "UTC" }),
    headers: { Authorization: `Bearer ${session.session.access_token}` },
  }), { params: Promise.resolve({}) });
  assert.equal(liveResponse.status, 200);
  assert.match(liveResponse.headers.get("Cache-Control") ?? "", /no-store/);
  assert.doesNotMatch(liveResponse.headers.get("Server-Timing") ?? "", /maintenance/);
  assert.deepEqual(fightSnapshotSchema.parse(await liveResponse.json()).members, snapshots[0].members);

  const [beforeNoop] = await database`select count(*)::int as count from realtime.messages
  ...
  const [afterNoop] = await database`select count(*)::int as count from realtime.messages
    where topic = ${"fitfight:fights:" + owner} and event = 'fights_changed'`;
  assert.equal(afterNoop.count, beforeNoop.count, "An unchanged rank or score write must not broadcast again");
  await database`update public.fight_members set last_synced_at = now() where fight_id = ${fightId}`;
  const [afterSync] = await database`select count(*)::int as count from realtime.messages
    where topic = ${"fitfight:fights:" + owner} and event = 'fights_changed'`;
  assert.equal(afterSync.count, beforeNoop.count + 1, "Sync freshness changes notify each participant once per statement");
  ...
  const rejoined = clients[1].channel(`fitfight:fights:${peer}`, { config: { private: true } });
  await subscribe(rejoined, true);
  const reconciled = await readFightSnapshot(peer, "UTC", database);
  assert.equal(reconciled.members.find((member) => member.user_id === peer)?.current_value, 30000);

  const previous = received[0];
  await database`delete from public.fight_members where fight_id = ${fightId} and user_id = ${owner}`;
  for (let attempt = 0; attempt < 100 && received[0] === previous; attempt++) await delay(50);
  assert.ok(received[0] > previous, "A removed participant must be told to reread their access");
  assert.deepEqual((await readFightSnapshot(owner, "UTC", database)).members, []);
  assert.deepEqual((await readFightSnapshot(outsider, "UTC", database)).fights, []);
});
