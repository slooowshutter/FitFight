...
test("reaction people respect membership, blocks, deletion, and pagination", async (t) => {
  ...
  for (const viewer of [owner, accepted, deferred]) {
    const first = await listFightPostReactionPeople(viewer, postId, { limit: 1 }, database);
    assert.equal(first.people[0].user_id, expected[0]);
    assert.equal(first.next_cursor, expected[0]);
    const second = await listFightPostReactionPeople(viewer, postId, { limit: 1, cursor: first.next_cursor }, database);
    assert.equal(second.people[0].user_id, expected[1]);
    assert.equal(second.next_cursor, null);
    assert.deepEqual(new Set([first.people[0].emoji, second.people[0].emoji]), new Set(["❤️", "🔥"]));
  }
  for (const viewer of [invited, outsider]) {
    await assert.rejects(listFightPostReactionPeople(viewer, postId, { limit: 40 }, database), { status: 403 });
  }
  await assert.rejects(listFightPostReactionPeople(owner, randomUUID(), { limit: 40 }, database), { status: 404 });

  await database`insert into private.feed_blocks (blocker_id, blocked_id) values (${owner}, ${accepted})`;
  const filtered = await listFightPostReactionPeople(owner, postId, { limit: 1 }, database);
  assert.deepEqual(filtered.people.map((person) => person.user_id), [deferred]);
  ...
  assert.deepEqual(await listFightPostReactionPeople(accepted, postId, { limit: 40 }, database), {
    people: [], next_cursor: null,
  });
  await database`update public.profiles set deleted_at = now() where user_id = ${deferred}`;
  assert.deepEqual(await listFightPostReactionPeople(owner, postId, { limit: 40 }, database), {
    people: [], next_cursor: null,
  });
});
