...
function recordingDatabase(
    ...
    const sql = ((strings: TemplateStringsArray, ...values: unknown[]) => {
        ...
        return Promise.resolve(
            text.includes("returning to_char")
    ...
    sql.begin = ((
        _mode: string,
        callback: (transaction: Sql) => Promise<unknown>,
...
test("HealthKit attempt timings preserve fractional elapsed time and server durations", () => {
    assert.deepEqual(
        healthKitSyncAttemptSchema.parse(validAttempt),
        validAttempt,
    );
    for (const outcome of ["failed", "cancelled"]) {
        ...
        assert.deepEqual(
            healthKitSyncAttemptSchema.parse(partialAttempt),
            partialAttempt,
        );
    }
});
...
test("HealthKit failures preserve safe native errors and reject device details", async () => {
    ...
    for (const invalid of [
        ...
        assert.throws(() =>
            healthKitSyncAttemptSchema.parse({
                ...attempt,
                stages: [{ ...attempt.stages[0], error: invalid }],
            }),
        );
    }
...
test("HealthKit timing boundaries reject invalid durations, counts and unknown fields", () => {
    ...
    for (const invalid of [
        ...
        { stages: [{ ...validAttempt.stages[0], stage: "unknown" }] },
        {
            stages: [
                {
                    ...validAttempt.stages[0],
                    server_timing: { query: "select private data" },
                },
            ],
        },
        ...
        assert.throws(() =>
            healthKitSyncAttemptSchema.parse({ ...validAttempt, ...invalid }),
        );
    }
    assert.throws(() =>
        healthKitDiagnosticSnapshotSchema.parse({
...
test("HealthKit diagnostics batch attempt inserts with authenticated ownership and deduplication", async () => {
    ...
    const attempts = [
        ...
        {
            ...validAttempt,
            attempt_id: "a8263691-af86-4119-83dc-a5aa34a7283d",
        },
    ];
