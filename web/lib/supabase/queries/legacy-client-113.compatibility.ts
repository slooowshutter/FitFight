...
test("public build 113 can name its profile, create, invite, accept, decline, and read after migration", async (t) => {
    ...
    const profiles = await owner.client
        .from("profiles")
        .select(contract.profile_select)
    ...
    assert.equal(
        (
            await owner.client.from("fight_members").insert([
                {
                    fight_id: fightId,
    ...
    assert.equal(
        (
            await stranger.client.from("fight_members").insert({
                fight_id: fightId,
        ...
        "42501",
        "strangers cannot join a private fight",
    );
    assert.equal(
        (
            await owner.client.from("fight_members").insert({
        ...
        "42501",
        "owners cannot accept for another person",
    );
    assert.equal(
        (
    ...
    const members = await invitee.client
        .from("fight_members")
        .select(contract.member_select)
        .eq("fight_id", fightId);
    ...
    for (const row of legacyMembership113Schema.array().parse(members.data)) {
        assert.deepEqual(
            ...
            contract.member_select.split(",").sort(),
        );
    ...
    const fights = await invitee.client
        .from("fights")
        .select(contract.fight_select)
        .eq("id", fightId)
        .single();
    assert.equal(fights.error, null);
    assert.ok(fights.data);
    assert.deepEqual(
        Object.keys(fights.data).sort(),
        contract.fight_select.split(",").sort(),
    );
    const steps = await invitee.client
        .from("step_days")
        .select(contract.steps_select)
    ...
    assert.equal(
        (
            await stranger.client
                .from("fight_members")
                .update(contract.decline_update)
                .eq("fight_id", fightId)
                .eq("user_id", stranger.userId)
        ).error,
        null,
    );
    const declined = await stranger.client
        .from("fight_members")
        .select("state")
        .eq("fight_id", fightId)
        .eq("user_id", stranger.userId)
