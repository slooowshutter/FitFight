...
test("media removal respects Storage's 1000-object limit without skipping paths", async (t) => {
    const paths = Array.from(
        { length: 2001 },
        (_, index) => `user-id/fight_post/${index}`,
    );
    ...
    t.mock.method(
        ...
        async (...[, request]: Parameters<typeof fetch>) => {
            const { prefixes } = z
                .object({ prefixes: z.array(z.string()) })
                .parse(JSON.parse(String(request?.body)));
            batches.push(prefixes);
            return new Response(
                JSON.stringify(
                    prefixes.length > 1000
...
test("media uploads accept only bounded photo metadata", () => {
    ...
    for (const input of [
        ...
        assert.equal(
            createMediaUploadRequestSchema.safeParse(input).success,
            false,
        );
    }
});
...
test("batch signed URLs keep one URL per object path and ignore failed rows", () => {
    const urls = signedUrlsFromBatch(
        ...
        [
            {
                path: "a/photo",
                signedUrl: "https://example.com/a?token=1",
                error: null,
            },
            {
                path: null,
                signedUrl: "https://example.com/b?token=2",
                error: null,
            },
    ...
    const media = mapMedia(
        {
            ...
            status: "ready",
            object_path:
                "11111111-1111-4111-8111-111111111111/profile/" + mediaId,
...
test("media routes authenticate before creating or committing an upload", async () => {
    const created = await createMedia(
        new Request("https://staging.fitfight.app/api/v1/media", {
            method: "POST",
        }),
        { params: Promise.resolve({}) },
    );
    assert.equal(created.status, 401);
    const committed = await commitMedia(
        new Request(
            "https://staging.fitfight.app/api/v1/media/" + mediaId + "/commit",
            {
                method: "POST",
            },
        ),
