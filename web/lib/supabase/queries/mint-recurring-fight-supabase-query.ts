...
const MEMBER_SELECT =
    "fight_id, user_id, state, selected_source_id, source_label";
...
export async function mintNextRecurringFight(
    ...
    await admin
        .from("fight_series")
        .update({ current_fight_id: nextId })
        .eq("id", series.id);
    return nextId;
}
