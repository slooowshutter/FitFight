...
const legacyProfile = profileSchema.parse(
    JSON.parse(
        readFileSync(
            new URL(
                "../../../../contracts/fixtures/profile.json",
                import.meta.url,
            ),
            "utf8",
        ),
...
test("profile patches normalize handles and reject caller-owned IDs, timestamps, and empty updates", () => {
    assert.deepEqual(
        updateProfileRequestSchema.parse({ handle: " @Marc_New@ " }),
        { handle: "marc_new" },
    );
    assert.deepEqual(
        updateProfileRequestSchema.parse({ display_name: " Marc " }),
        { display_name: "Marc" },
    );
    for (const input of [
        {},
    ...
    assert.deepEqual(
        updateProfileRequestSchema.parse({ companion_id: "fox" }),
        { companion_id: "fox" },
    );
    assert.deepEqual(
...
test("missing accounts and duplicate usernames return stable API errors", async () => {
    ...
    await assert.rejects(
        readProfile(profile.user_id, admin),
        (error: unknown) =>
    ...
    await assert.rejects(
        updateProfile(profile.user_id, { handle: "taken" }, admin),
        (error: unknown) =>
            error instanceof ApiError &&
            error.status === 409 &&
...
test("profile routes authenticate before reads, writes, and deletion and allow PATCH preflights", async () => {
    const { GET, PATCH, DELETE, OPTIONS } = await import(
        "@/app/api/v1/me/route"
    );
    for (const [method, handler] of [
        ...
        const response = await handler(
            new Request("https://fitfight.app/api/v1/me", { method }),
            {
    ...
    const preflight = OPTIONS(
        new Request("https://fitfight.app/api/v1/me", { method: "OPTIONS" }),
    );
    assert.match(
...
test("profile HTTP routes use the verified owner, validate patches, and reject deleted accounts", async (t) => {
    ...
    const token =
        [
            { alg: "HS256", typ: "JWT" },
            { sub: profile.user_id, exp: Math.floor(Date.now() / 1000) + 3600 },
        ]
            .map((part) =>
    ...
    const read = await GET(
        new Request("https://fitfight.app/api/v1/me", { headers }),
        context,
    );
    ...
    const companion = await PATCH(
        new Request("https://fitfight.app/api/v1/me", {
            method: "PATCH",
            headers,
            body: JSON.stringify({ companion_id: "fox" }),
        }),
        context,
    );
    assert.equal(companion.status, 200);
    assert.deepEqual(await companion.json(), {
        ...profile,
        display_name: "New Name",
        companion_id: "fox",
    });
    ...
    assert.deepEqual(await missing.json(), {
        error: "Invalid or deleted account",
