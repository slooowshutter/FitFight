...
export async function createProviderUpload(
    ...
    if (error || !data) {
        throw new ApiError(
            ...
            "Could not authorize archive upload",
        );
    }
...
export async function getProviderUpload(
    ...
    if (!upload) {
        throw new ApiError(
            ...
            "Provider upload was not found",
        );
    }
    return {
        upload_id: upload.upload_id,
        status: upload.status,
        ...(upload.status === "completed" && upload.receipt
...
export async function getProviderUploadContext(
    ...
    const rows = providerFightWindowRowSchema.array().parse(
        await database`
...
export async function processProviderUpload(
    ...
    const claimed = await database.begin("read write", async (sql) => {
        ...
        if (
            ...
            throw new ApiError(
                409,
                ERROR_CODES.upload_busy,
                "Provider upload is processing",
            );
        }
        await sql`
    ...
    if (claimed.status !== "committed" && claimed.status !== "completed") {
        ...
        if (error || !data) {
            ...
            throw new ApiError(
                ...
                errorCode,
                missing
                    ? "Archive object was not found"
                    : "Could not download archive object",
            );
        }
        ...
        try {
            const { data: eventStream, error: eventStreamError } =
                await admin.storage
                    .from(BUCKET)
                    .download(claimed.object_path)
                    .asStream();
            if (eventStreamError || !eventStream) {
                throw new ApiError(
                    503,
                    ERROR_CODES.storage_error,
                    "Could not reread archive object",
                );
            }
            ...
            `;
                ...
                const mergedDays = archive.records.filter(
                    (item) => item.record.type === "merged_day",
                );
                const sourceDays = archive.records.filter(
                    (item) => item.record.type === "source_day",
                );
                const fightAggregates = archive.records.filter(
                    (item) => item.record.type === "fight_aggregate",
                );
                const checkpoint = archive.records.find(
                    (item) => item.record.type === "checkpoint",
                );
                if (!checkpoint || checkpoint.record.type !== "checkpoint") {
                    throw new ApiError(
                        400,
                        ERROR_CODES.archive_invalid,
                        "Archive checkpoint is missing",
                    );
                }
                ...
                `;
                    const changedSampleIds = new Set(
                        previousSamples.map((row) => row.external_record_id),
                    );
                    const replayKinds = new Map(
                        previousSamples.map((row) => [
                    ...
                    const rows = eventBatch.map((item) => {
                        ...
                        return {
                            ...
                            event_kind:
                                record.type === "sample"
                                    ? (replayKinds.get(
                                          `${record.sample_id}:${item.inputHash}`,
                                      ) ??
                                      (changedSampleIds.has(record.sample_id)
                                          ? "change"
                                          : "add"))
                                    : "delete",
                ...
                `;
                    if (!fight) {
                        throw new ApiError(
                            400,
                            ERROR_CODES.archive_invalid,
                            "Fight aggregate does not match context",
                        );
                    }
                    await sql`
                    insert into private.metric_observations (
                ...
                `;
                    const scores = scoreFight({
                        ...
                        members: members.map((member) => ({
                            ...
                            value:
                                asNumber(member.current_value) ??
                                asNumber(member.final_value) ??
                                0,
            ...
            const errorCode =
                error instanceof ApiError ? error.code : ERROR_CODES.db_error;
            await database`
                update private.provider_uploads
            ...
            throw new ApiError(
                500,
                ERROR_CODES.db_error,
                "Could not process provider archive",
            );
        }
    }
    ...
    if (
        ...
        return {
            response: await getProviderUpload(userId, uploadId, database),
            cleanupPending: true,
        };
    }
    await database`
    ...
    if (paths.length > 0) {
        ...
        if (
            ...
            throw new ApiError(
                503,
                ERROR_CODES.storage_error,
                "Could not remove pending provider archives",
            );
        }
    }
}
