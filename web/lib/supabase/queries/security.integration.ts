...
async function fixture(t: TestContext) {
    ...
    const startsAt = new Date(
        Date.parse(endsAt) - 3 * 86_400_000,
    ).toISOString();
    t.after(async () => {
...
test("diagnostic reports persist JSON timing arrays once per user and attempt", async (t) => {
    ...
    assert.deepEqual(rows[0], {
        user_id: f.owner,
        total_ms: 125.5,
        stages: input.attempts?.[0].stages,
    });
});
...
test("a source watermark cannot finalize a fight without exact end snapshots", async (t) => {
    ...
    const [pending] =
        await database`select state from public.fights where id = ${f.fightId}`;
    ...
    `;
    assert.ok(
        pendingMembers.every(
            (row) => !row.final_steps_complete && row.final_value === null,
        ),
    );
    await recalculateFight(
        f.fightId,
        new Date(Date.parse(f.endsAt) + 86_400_001),
        database,
    );
    ...
    `;
    assert.deepEqual(
        finalized.map((row) => row.final_value),
        ["42", "0"],
    );
    assert.deepEqual(
        finalized.map((row) => row.rank),
        [1, 1],
    );
    assert.ok(
        finalized.every(
            (row) => !row.final_steps_complete && row.finalized_at !== null,
        ),
    );
});
...
test("latest selected-source exact snapshots freeze together and reject later uploads", async (t) => {
    ...
    for (const [index, [userId, steps]] of readings.entries()) {
        await syncHealthKitAggregates(
            userId,
            {
                complete_through: new Date(
                    f.now.getTime() - 1000 + index,
                ).toISOString(),
                time_zone: "UTC",
                merged_days: [],
                fight_aggregates: [
    ...
    `;
    assert.deepEqual(
        before.map((row) => row.final_value),
        ["10", "30"],
    );
    assert.deepEqual(
        before.map((row) => row.rank),
        [2, 1],
    );
    assert.ok(
        before.every(
    ...
    await assert.rejects(
        syncHealthKitAggregates(
            f.owner,
            {
                complete_through: f.now.toISOString(),
                time_zone: "UTC",
                merged_days: [],
                fight_aggregates: [
                    {
...
test("batched uploads preserve independent fight scores, corrections, replay, and overlapping rosters", async (t) => {
    ...
    for (const [index, [firstSteps, secondSteps]] of ownerReadings.entries()) {
        ...
        await syncHealthKitAggregates(
            f.owner,
            {
                complete_through: checkpoint,
                time_zone: "UTC",
                merged_days: [],
                fight_aggregates: [
                    {
                        fight_id: secondFightId,
                        starts_at: f.startsAt,
                        ends_at: secondEndsAt,
                        cutoff_at: checkpoint,
                        steps: secondSteps,
                    },
                    {
                        fight_id: f.fightId,
    ...
    await Promise.all([
        syncHealthKitAggregates(
            f.owner,
            {
                complete_through: replayCheckpoint,
                time_zone: "UTC",
                merged_days: [],
                fight_aggregates: [
                    {
    ...
    assert.ok(
        snapshots.every((row) => row.count === 3),
        "replay must not create another snapshot",
    );
});
...
test("a failure midway through finalization rolls back all scores and the fight state", async (t) => {
    ...
    try {
        await assert.rejects(
            recalculateFight(f.fightId, f.now, database),
            /security_test_finalization_failure/,
        );
        const [fight] =
            await database`select state from public.fights where id = ${f.fightId}`;
...
test("declining requires the caller's invited membership and revokes its invite", async (t) => {
    const f = await fixture(t);
    await assert.rejects(
        declineMembership(randomUUID(), f.fightId, database),
        /not invited/,
    );
    await assert.rejects(
        declineMembership(f.owner, f.fightId, database),
        /cannot be declined/,
    );
    await database`
    ...
    `;
    const [invite] =
        await database`select revoked_at from public.fight_invites where fight_id = ${f.fightId}`;
...
test("deleting an owner with other participants removes their account and private data", async (t) => {
    ...
    assert.equal(
        (await database`select id from auth.users where id = ${f.owner}`)
            .length,
        0,
    );
    assert.equal(
        (await database`select id from public.fights where id = ${f.fightId}`)
            .length,
        0,
    );
    assert.equal(
        (
            await database`select id from public.fight_series where id = ${seriesId}`
        ).length,
        0,
    );
    assert.equal(
        (await database`select id from auth.users where id = ${f.peer}`).length,
        1,
    );
    assert.equal(
        (await database`select id from public.fights where id = ${peerFightId}`)
            .length,
        1,
    );
});
