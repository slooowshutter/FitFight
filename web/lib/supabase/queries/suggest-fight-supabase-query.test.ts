...
test("suggest fight is a boolean flag and rejects extra fields", () => {
  assert.deepEqual(suggestFightRequestSchema.parse({ suggested: true }), { suggested: true });
  assert.deepEqual(suggestFightRequestSchema.parse({ suggested: false }), { suggested: false });
  assert.equal(suggestFightRequestSchema.safeParse({}).success, false);
  assert.equal(suggestFightRequestSchema.safeParse({ suggested: true, extra: 1 }).success, false);
});
...
test("suggest fight routes authenticate before reading or writing", async () => {
  ...
  const listed = await listSuggested(new Request("https://staging.fitfight.app/api/v1/fights/suggested"), {
    params: Promise.resolve({}),
  });
  const updated = await suggestFight(new Request(`https://staging.fitfight.app/api/v1/fights/${fightId}/suggested`, {
    method: "PATCH",
