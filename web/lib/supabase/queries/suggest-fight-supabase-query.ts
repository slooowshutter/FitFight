...
export async function setFightSuggested(
  ...
  if (!isFitFightAdmin(viewer)) {
    throw new ApiError(403, ERROR_CODES.forbidden, "Only Marc can suggest a fight");
  }
  const fight = await loadFight(fightId, admin);
  if (!fight.series_id) {
    throw new ApiError(409, ERROR_CODES.conflict, "This fight cannot be suggested");
  }
  ...
  if (error) {
    throw new ApiError(500, ERROR_CODES.db_error, "Could not update that suggestion");
  }
  return { suggested };
}
