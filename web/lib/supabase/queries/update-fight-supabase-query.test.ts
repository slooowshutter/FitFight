...
test("owner update rejects an empty patch and a reversed window", () => {
  ...
  assert.equal(updateFightRequestSchema.safeParse({ inviteHandles: [] }).success, false);
  assert.equal(
    updateFightRequestSchema.safeParse({ startsAt: endsAt, endsAt: startsAt }).success,
    false,
  );
});
...
test("owner can rename a live fight without touching membership", async (t) => {
  ...
  process.env.NEXT_PUBLIC_SUPABASE_URL = process.env.NEXT_PUBLIC_SUPABASE_URL || "https://update-fight.example";
  process.env.SUPABASE_SECRET_KEY = process.env.SUPABASE_SECRET_KEY || "test-only-key";
  t.after(() => {
  ...
  const result = await updateFight(
    owner,
    fightId,
    updateFightRequestSchema.parse({ name: "Office steps", visibility: "invite_only" }),
  );
...
test("only the owner can edit, and finished fights stay frozen", async (t) => {
  ...
  await assert.rejects(
    () => updateFight(other, fightId, updateFightRequestSchema.parse({ name: "Nope" })),
    (error: unknown) => error instanceof ApiError && error.status === 403,
  );
  await assert.rejects(
    () => updateFight(owner, fightId, updateFightRequestSchema.parse({ name: "Nope" })),
    (error: unknown) =>
      error instanceof ApiError && error.status === 409 && error.message.includes("no longer be edited"),
  );
});
...
test("the owner cannot kick themselves", async (t) => {
  ...
  const { updateFight } = await import("./update-fight-supabase-query");
  await assert.rejects(
    () => updateFight(owner, fightId, updateFightRequestSchema.parse({ removeUserIds: [owner] })),
    (error: unknown) =>
      error instanceof ApiError && error.status === 400 && error.message.includes("owner cannot be removed"),
  );
});
  ...
  await assert.rejects(
    () =>
      updateFight(
        owner,
        fightId,
        updateFightRequestSchema.parse({ startsAt: "2026-09-15T12:00:00.000Z" }),
      ),
    (error: unknown) =>
