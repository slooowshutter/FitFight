...
import type { FightMemberRow, FightMemberState, FightState } from "@/lib/types/database";
...
import { fightSummary, loadFight, loadOwnedFight, loadSeries } from "./fight-access-supabase-query";
...
const ACTIVE_MEMBER_STATES: FightMemberState[] = ["invited", "accepted", "deferred"];
...
export async function updateFight(
  ...
  switch (fight.state) {
    ...
    case "final":
    case "cancelled":
      throw new ApiError(409, ERROR_CODES.conflict, "This fight can no longer be edited");
    default: {
  ...
  const inviteHandles = [...new Set((input.inviteHandles ?? []).map((handle) => handle.trim()).filter(Boolean))];
  if (removeIds.includes(fight.owner_id)) {
    throw new ApiError(400, ERROR_CODES.validation, "The owner cannot be removed from this fight");
  }

  const nextStarts = input.startsAt ? new Date(input.startsAt) : new Date(fight.starts_at);
  const nextEnds = input.endsAt ? new Date(input.endsAt) : new Date(fight.ends_at);
  if (input.startsAt !== undefined && fight.state !== "scheduled") {
    throw new ApiError(400, ERROR_CODES.validation, "Start time can only change before the fight begins");
  }
  if (nextEnds.getTime() <= nextStarts.getTime()) {
    throw new ApiError(400, ERROR_CODES.validation, "The end must be after the start");
  }
  if (nextEnds.getTime() <= now.getTime()) {
    throw new ApiError(400, ERROR_CODES.validation, "The end must be in the future");
  }
  if (input.startsAt !== undefined && nextStarts.getTime() <= now.getTime()) {
    throw new ApiError(400, ERROR_CODES.validation, "Choose a start time in the future");
  }

  const identityChanged = input.name !== undefined || input.actionText !== undefined;
  const stored = identityChanged
    ? storedFightIdentity(
        input.name !== undefined ? input.name : fight.name,
        input.actionText !== undefined ? input.actionText : (fight.action_text ?? undefined),
      )
    : null;
  const windowChanged =
    nextStarts.getTime() !== Date.parse(fight.starts_at) || nextEnds.getTime() !== Date.parse(fight.ends_at);
  ...
  if (Object.keys(fightPatch).length > 0) {
    const { error } = await admin.from("fights").update(fightPatch).eq("id", fightId);
    if (error) {
      throw new ApiError(500, ERROR_CODES.db_error, "Could not update fight");
    }
  }
  ...
  if (fight.series_id) {
    ...
    if (windowChanged) {
      seriesPatch.duration_seconds = Math.round((nextEnds.getTime() - nextStarts.getTime()) / 1000);
    }
    if (Object.keys(seriesPatch).length > 0) {
      const { error } = await admin.from("fight_series").update(seriesPatch).eq("id", fight.series_id);
      if (error) {
        throw new ApiError(500, ERROR_CODES.db_error, "Could not update fight");
      }
    }
  ...
  for (const removeId of removeIds) {
    ...
    const member = memberData as Pick<FightMemberRow, "user_id" | "state"> | null;
    ...
    const { error: withdrawError } = await admin
      ...
      .eq("user_id", removeId);
    if (withdrawError) {
      throw new ApiError(500, ERROR_CODES.db_error, "Could not remove that person");
    }
    const { error: revokeError } = await admin
      ...
      .is("revoked_at", null);
    if (revokeError) {
      throw new ApiError(500, ERROR_CODES.db_error, "Could not revoke that invite");
    }
    if (fight.series_id) {
